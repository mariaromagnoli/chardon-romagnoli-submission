...
\noindent A similar risk is to use already-treated units as controls for later cohorts \citep{GoodmanBacon2021}. To limit this bias, occupations listed since 2008 are excluded from both groups. However, occupation$\times$region cells listed in 2021 remain in the 2025 control group while being already treated by then. Fixed effects may absorb their time-varying wage trajectory, but this does not eliminate the bias\footnote{Excluding 2021-listed cells from the 2025 control group would perhaps be a cleaner solution, but we were not sure of the implications. Reflection on this is left for future work.}.
...
\noindent If employer bargaining power over non-EEA workers increases in listed sectors, we would expect \texttt{Treat$\,\times\,$Post$^{\text{non-EEA}} <$ Treat$\,\times\,$Post$^{\text{EEA}}$}. The point estimates go in the opposite direction (Table \ref{tab:did2021_fe3}). Both are insignificant. Results are consistent both in sign and magnitudes across fixed effect structures (Appendix C, Tables \ref{tab:did2021_fe1} and \ref{tab:did2021_fe2}).
...
\section{Conclusions}
\smallskip
\noindent We find no evidence of a monopsony effect. However, data misalignment and design constraints limited our analysis. Future work should correct data cleaning errors, remove previously treated occupations when studying subsequent waves, focus on similar occupations, and use different samples. Disentangling monopsony from hiring and competition effects requires alternative identification strategies.

\noindent Even we found no effect, we like to think our small contribution consists in our framework. Given Europe's demographic decline, labor economics research on migrant work should shift from assessing native wage impacts to understanding how to  attract and retain essential workers under fair and competitive conditions.
\clearpage
